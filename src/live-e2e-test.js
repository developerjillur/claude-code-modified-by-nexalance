// Live end-to-end test of the INSTALLED hook at ~/.claude/
// (not the bundled-in-repo copy). Exercises full sequences of queue
// adds, hook fires, and verifies drainage, source-tagging, workspace
// isolation, and the v0.3.0 feedback-only flow.
...
const wsRaw  = path.join(os.tmpdir(), 'claude-mod-live-e2e-' + Date.now() + '-' + Math.floor(Math.random() * 100000));
const wsBRaw = path.join(os.tmpdir(), 'claude-mod-live-e2e-B-' + Date.now() + '-' + Math.floor(Math.random() * 100000));
fs.mkdirSync(wsRaw, { recursive: true });
fs.mkdirSync(wsBRaw, { recursive: true });
const ws  = fs.realpathSync(wsRaw);
const wsB = fs.realpathSync(wsBRaw);

try {
	group('Hook exists at stable path and is registered');
	...
	assert(!settings.hooks?.UserPromptSubmit || !JSON.stringify(settings.hooks.UserPromptSubmit).includes('claude-mod-user-submit-hook'), 'v0.3.0 — no UserPromptSubmit hook entry installed');
	const stopCmd = JSON.stringify(settings.hooks?.Stop || []);
	assert(!stopCmd.includes('CLAUDE_MOD_ENABLE_NATIVE'), 'no CLAUDE_MOD_ENABLE_NATIVE env var on Stop command');
	assert(!stopCmd.includes('CLAUDE_MOD_DISABLE_NATIVE'), 'no CLAUDE_MOD_DISABLE_NATIVE env var on Stop command');
	...
	group('Multi-item drainage across Stop chain');
	nukeWorkspaceDir(ws);
	const items = [
	...
	try { fs.symlinkSync(ws, linkPath); } catch (_) { /* skip */ }
	...
	group('Malformed JSON in queue file → safe fallback');
	nukeWorkspaceDir(ws);
	...
	group('Consecutive-block safety cap');
	nukeWorkspaceDir(ws);
	if (!fs.existsSync(wsDir(ws))) fs.mkdirSync(wsDir(ws), { recursive: true });
	...
	group('Full simulated session — drain across chain to idle');
	nukeWorkspaceDir(ws);
	writeQueue(ws, [
	...
	nukeWorkspaceDir(ws);
	nukeWorkspaceDir(wsB);
	try { fs.rmdirSync(wsRaw); } catch (_) {}
	try { fs.rmdirSync(wsBRaw); } catch (_) {}
}
